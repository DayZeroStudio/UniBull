...
module.exports = function() {
    ...
    var modelsList = ["Class", "Thread", "User", "Reply", "ClassesUsers"];
    return Promise.resolve(modelsList).map(function(model) {
        ...
        var fs = require("fs");
        var path = require("path");
        this.defaults = JSON.parse(fs.readFileSync(path.join(__dirname, "/defaults.json"), "utf-8"));
        return this.defaults.classes;
    }).map(function(klass) {
        return dbModels.Class.create(klass).then(function(newKlass) {
            if (klass.threads) {
                return Promise.resolve(klass.threads).map(function(thread) {
                    dbModels.Thread.create(thread).then(function(newThread) {
                        newKlass.addThread(newThread);
                    });
                });
            }
        });
